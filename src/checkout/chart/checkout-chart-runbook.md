...
```bash
# Replace <ELASTICACHE_ENDPOINT> before running.
helm upgrade --install checkout src/checkout/chart/ \
  -f src/checkout/chart/values.yaml \
...
```bash
# Replace <ELASTICACHE_TLS_ENDPOINT> before running.
helm upgrade --install checkout src/checkout/chart/ \
  -f src/checkout/chart/values.yaml \
...
helm upgrade --install checkout src/checkout/chart/ \
  -f src/checkout/chart/values.yaml \
  -f src/checkout/chart/values-hpa.yaml \
  --namespace checkout --create-namespace
...
helm upgrade --install checkout src/checkout/chart/ \
  -f src/checkout/chart/values.yaml \
  -f src/checkout/chart/values-pdb.yaml \
  --namespace checkout --create-namespace
...
| HPA | Metrics server required in cluster |
